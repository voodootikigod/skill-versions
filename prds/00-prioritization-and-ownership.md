...
- **🔧 skills-check** — Fits as an extension of skillscheck.ai. These are tools that *analyze, verify, and maintain* skills. They're about the health and currency of skill content. This is your project.
...
|---|-----------|---------|--------|-------|-----------|
| 1 | **Security Audit & Hallucination Detection** | ★★★★★ | ★★★★★ | 🔧 skills-check | Active threat vector (Aikido research). LLM hallucination of package names is a novel attack surface. No existing tool addresses this. |
...
| 3 | **Skill Testing / Eval Integration** | ★★★★☆ | ★★★★★ | 🤝 Joint (spec + skills-check + skills.sh) | Building blocks exist (OpenAI evals, Vercel agent-eval, Anthropic methodology) but nobody has connected them to the skill format. Integration is the opportunity. |
| 4 | **Context Budget Analysis** | ★★★★★ | ★★★★☆ | 🔧 skills-check | Completely novel. No equivalent in any package ecosystem. Token cost of skills + transitive dependencies is a real constraint that only matters for LLM-loaded artifacts. |
| 5 | **Semver for Knowledge / Version Verification** | ★★★★☆ | ★★★★☆ | 🔧 skills-check | Extends your existing product-version work. Automated verification that skill content changes match the claimed version bump. cargo semver-checks adapted for knowledge artifacts. |
...
| 7 | **Policy Enforcement (cargo deny for skills)** | ★★★☆☆ | ★★★★☆ | 🔧 skills-check | Adapted from cargo deny. Enterprise gate-keeper: enforce trust policies, ban sources, require metadata. Blocks adoption without it. |
...
1. Security Audit & Hallucination Detection → **skills-check**
2. Context Budget Analysis → **skills-check**
3. Semver for Knowledge / Version Verification → **skills-check**
...
6. Policy Enforcement → **skills-check**
7. Mandatory Metadata validation → **skills-check** (lint/check) + **skills.sh** (enforce on publish)
...
11. Skill Testing CLI integration → **skills-check** (for regression after refresh)
...
## What Lives in skills-check vs. skills.sh

The skills.sh maintainer is a colleague/co-worker. skills-check should **leverage and complement** skills.sh, never duplicate or compete with it. PRDs tagged as "propose to skills.sh" are written proposals or PRs to bring to that maintainer — not features to build in skills-check.

### skills-check: "The Quality & Integrity Layer"

skills-check answers: **"Are my skills correct, safe, current, and efficient?"**

Build in skills-check:
...
Propose to skills.sh (don't build in skills-check):
- Lockfiles
- Deprecation & yanking
...
This three-layer split (spec defines the contract, skills-check verifies compliance, skills.sh handles distribution) mirrors how Rust works: the language spec defines features, `cargo` handles distribution, and tools like `clippy`/`cargo-deny`/`cargo-semver-checks` handle quality verification.

Where skills-check connects to skills.sh:
- skills-check reads skills.sh's installed skill directories (don't maintain a parallel inventory)
- skills-check could run as a pre-install hook or post-install check in skills.sh
- skills-check's `check` output could feed into `npx skills status`
- skills-check's `policy` source rules reference skills.sh registry sources
- skills.sh lifecycle data (deprecation, yank status) feeds into skills-check's health reports
