...
Policy-as-code enforcement via `.skill-policy.yml`. Seven validators: source allow/deny with glob matching, required skills verification, banned skills, metadata requirements (reuses lint's SPDX validation), content deny/require patterns with line numbers, freshness/staleness limits, and audit integration (cross-command: runs `runAudit()` when `audit.require_clean` is configured). Policy file discovery walks up directories for monorepo support. `policy/signature.ts` provides detached Ed25519 signatures (sidecar `.skill-policy.yml.sig` over the raw file bytes): `policy sign` writes the sidecar, and `policy check --require-signature --pubkey <key>` verifies it **fail-closed** (a missing or mismatched signature aborts the check with exit 2 before any rule is trusted). When the policy uses `extends`, verification covers the **entire inheritance closure** — `collectPolicyFiles()` enumerates the child plus every transitive base, and each must carry a valid signature, so an unsigned or tampered base cannot be trusted.

**Inheritance (`extends`).** A policy may list base policies via `extends: ./base.yml` (or an array). `loadPolicyFile` resolves them in `policy/merge.ts`: scalars and nested object flags are child-overridden, arrays (sources lists, banned, required, content patterns, exemptions) accumulate as a union, and `version` comes from the child. Resolution is depth-first with per-branch cycle detection (diamonds allowed, cycles rejected); `extends` is stripped from the merged result. When `--require-signature` is used, the whole inheritance closure (child + all bases, via `collectPolicyFiles()`) must be signed — not just the leaf.

**Exemptions.** `exemptions:` are time-boxed waivers (`rule` + required `reason`, optional `skill` glob and `expires` ISO date). `policy/exemptions.ts` partitions findings into kept vs. exempted after all validators run; suppressed findings are recorded in `PolicyReport.exempted` and surfaced by reporters (never silent), and an expired exemption stops suppressing and emits an `exemption.expired` warning. Unparseable `expires` is treated as inactive (fail-closed). `rule: "*"` waives any rule; an exemption with no `skill` applies to any skill, while a `skill` glob only matches findings whose file maps to that skill name.
